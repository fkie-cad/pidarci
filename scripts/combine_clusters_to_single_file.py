...
from config import DATA_DIR, TEST_DIR, ROOT

OUTPUT_DIR = ROOT / 'patterns'
...
def _read_parsed_pattern_from_cluster_file(cluster_file_path: pathlib.Path) -> Tuple[List, str]:
    ...
    if is_trivial_cluster(data):
        return None, None
    return parsed_pattern, cluster_number_str


def is_trivial_cluster(data: Dict):
    cluster = data.get('cluster', [])
    original_constants = [x['original constant'] for x in cluster]
    if any(c in {1, 0, -1} for c in original_constants):
        return True
...
def combine_clusters_of_single_optimization_level(opt_level_path: pathlib.Path, output_path: pathlib.Path):
    ...
    for cluster_file in opt_level_path.glob("*"):
        ...
        if not parsed_pattern and not cluster_number_str:
            continue
    ...
    with (OUTPUT_DIR / output_path).open('w') as f:
...
def write_all_optimizations_pro_operation(operation_name: str, operation_folder: pathlib.Path):
    for opt_level in operation_folder.glob('*'):
        combine_clusters_of_single_optimization_level(operation_folder / opt_level.name,
                                                      pathlib.Path(f"patterns-{operation_name}-{opt_level.name}.json"))
